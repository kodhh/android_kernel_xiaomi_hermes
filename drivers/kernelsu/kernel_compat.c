...
__weak size_t copy_to_iter(const void *addr, size_t bytes, struct iov_iter *i)
{
	size_t total = min(bytes, i->count);
	size_t copied = 0;

	while (copied < total) {
		const struct iovec *iov = i->iov;
		size_t seg = min(total - copied, iov->iov_len - i->iov_offset);
		if (copy_to_user(iov->iov_base + i->iov_offset, addr + copied, seg))
			break;
		copied += seg;
		i->iov_offset += seg;
