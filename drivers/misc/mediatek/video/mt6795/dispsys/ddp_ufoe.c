...
static void ufoe_dump(void)
{
	...
	unsigned int ufoe_start = DISP_REG_GET(DISP_REG_UFO_START);
	unsigned int ufoe_pad = DISP_REG_GET(DISP_REG_UFO_CR0P6_PAD);
	unsigned int ufoe_width = DISP_REG_GET(DISP_REG_UFO_FRAME_WIDTH);
	unsigned int ufoe_height = DISP_REG_GET(DISP_REG_UFO_FRAME_HEIGHT);
	unsigned int ufoe_cfg0 = DISP_REG_GET(DISP_REG_UFO_CFG_0B);
	unsigned int ufoe_cfg1 = DISP_REG_GET(DISP_REG_UFO_CFG_1B);

	pr_debug("(0x000)UFOE_START =0x%x\n", ufoe_start);
	pr_debug("(0x020)UFOE_PAD  =0x%x\n", ufoe_pad);
	pr_debug("(0x050)UFOE_WIDTH =0x%x\n", ufoe_width);
	pr_debug("(0x054)UFOE_HEIGHT =0x%x\n", ufoe_height);
	pr_debug("(0x100)UFOE_CFG0 =0x%x\n", ufoe_cfg0);
	pr_debug("(0x104)UFOE_CFG1 =0x%x\n", ufoe_cfg1);
}
			...
			if (lcm_config->ufoe_params.lr_mode_en == 1) {
				...
				if (lcm_config->ufoe_params.compress_ratio == 3) {
					...
					if (internal_width % 6 != 0) {
						DISP_REG_SET_FIELD(handle, CR0P6_PAD_FLD_DISP_UFO_STR_PAD_NUM,
				...
				if (lcm_config->ufoe_params.vlc_disable) {
					DISP_REG_SET_FIELD(handle, CFG_0B_FLD_DISP_UFO_CFG_VLC_EN, DISP_REG_UFO_CFG_0B,
							   0);
