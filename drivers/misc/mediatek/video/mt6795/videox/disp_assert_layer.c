...
{
	DAL_LOG("%s, 64: layerVA=0x%lx, layerPA=0x%lx\n", __func__, layerVA, layerPA);

	dal_fb_addr = (void *)layerVA;
	dal_fb_pa = layerPA;

	/* DAL_LOG("%s, 32: layerVA=0x%08x, layerPA=0x%08x\n", __func__, dal_fb_addr, dal_fb_pa); */
...
DAL_STATUS DAL_Dynamic_Change_FB_Layer(unsigned int isAEEEnabled)
{
	...
	if (isAEEEnabled == 1) {
		...
		memcpy((void *)(&cached_layer_config[DISP_DEFAULT_UI_LAYER_ID]),
		       (void *)(&cached_layer_config[DISP_CHANGED_UI_LAYER_ID]), sizeof(OVL_CONFIG_STRUCT));
...
DAL_STATUS DAL_Printf(const char *fmt, ...)
{
	...
	DAL_STATUS ret = DAL_STATUS_OK;
	DAL_LOG("%s", __func__);
	/* DAL_LOG("[MTKFB_DAL] DAL_Printf mfc_handle=0x%08X, fmt=0x%08X\n", mfc_handle, fmt); */
	if (NULL == mfc_handle)
		return DAL_STATUS_NOT_READY;
