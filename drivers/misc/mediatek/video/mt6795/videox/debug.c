...
/* temporarily enable more log for debug */
...
static void dump_layer_info(void)
{
	unsigned int i;
	for (i = 0; i < 4; i++) {
		DISPMSG("LayerInfo in LCD driver, layer=%d,layer_en=%d, source=%d, fmt=%d, addr=0x%lx, x=%d, y=%d,",
		...
		DISPMSG(" w=%d, h=%d, pitch=%d, keyEn=%d, key=%d, aen=%d, alpha=%d\n",
...
void DBG_OnLcdDone(void)
{
	...
	if (fps.trigger_lcd_count >= dbg_opt.log_fps_wnd_size) {
		...
		DISP_LOG_PRINT(ANDROID_LOG_INFO, "DBG", "(TE delay %ld.%02ld ms, LCD %ld.%02ld ms)\n",
...
static void process_dbg_opt(const char *opt)
{
	...
	if (0 == strncmp(opt, "idle_switch_DC", 14)) {
		...
		kstrtoul(p + 1, 10, &value);
		int x = (int)value;
		DISPMSG("will set option %d to %d\n", option, x);
		disp_helper_set_option(option, x);
		...
		char *p = (char *)opt + 9;
		kstrtoul(p, 10, &value);
		unsigned long keep = value;
		kstrtoul(p + 1, 10, &value);
		unsigned long skip = value;
		...
		char *p = (char *)opt + 5;
		kstrtoul(p, 10, &value);
		g_lcm_x = (unsigned int)value;
	} else if (0 == strncmp(opt, "lcmy:", 5)) {
		char *p = (char *)opt + 5;
		kstrtoul(p, 10, &value);
		g_lcm_y = (unsigned int)value;
		...
		char *p = (char *)opt + 9;
		kstrtoul(p, 10, &value);
		UINT32 clk = (UINT32)value;
		DSI_ChangeClk(DISP_MODULE_DSI0, clk);
	} else if (0 == strncmp(opt, "dsidual_clk:", 12)) {
		/* This can't be used when screen update is running, because we use cpu to re-init MIPITX Clock,
		which can't ensure 2 dsi's clock is synchronous. */
		char *p = (char *)opt + 12;
		kstrtoul(p, 10, &value);
		UINT32 clk = (UINT32)value;
		...
		if (addr)
			DISPMSG("Read register 0x%p: 0x%08x\n", addr, INREG32(addr));
		else
			return;
		...
		char *p = (char *)opt + 9;
		kstrtoul(p, 10, &value);
		unsigned int maxlayer = (unsigned int)value;
		if (maxlayer)
		...
		char *p = (char *)opt + 10;
		kstrtoul(p, 10, &value);
		unsigned int enable = (unsigned int)value;
		...
		if (0 == strncmp(opt + 11, "on", 2)) {
			char *p = (char *)opt + 14;
			kstrtoul(p, 10, &value);
			gCapturePriLayerDownX = (unsigned int)value;
			kstrtoul(p + 1, 10, &value);
			gCapturePriLayerDownY = (unsigned int)value;
			kstrtoul(p + 1, 10, &value);
			gCapturePriLayerNum = (unsigned int)value;
...
#error
	if (0 == strncmp(opt, "hdmion", 6)) {
		...
		char *p = (char *)opt + 4;
		kstrtoul(p, 10, &value);
		unsigned int level = (unsigned int)value;
		...
		kstrtoul(p, 16, &value);
		lrs = (unsigned int)value;
		p++;
		kstrtoul(p, 16, &value);
		lgs = (unsigned int)value;
		p++;
		kstrtoul(p, 16, &value);
		lbs = (unsigned int)value;
		p++;
		kstrtoul(p, 16, &value);
		dbr = (unsigned int)value;
		p++;
		kstrtoul(p, 16, &value);
		dbg = (unsigned int)value;
		p++;
		kstrtoul(p, 16, &value);
		dbb = (unsigned int)value;
		...
		if (0 == strncmp(opt + 9, "on", 2)) {
			char *p = (char *)opt + 12;
			kstrtoul(p, 10, &value);
			gCaptureOvlDownX = (unsigned int)value;
			kstrtoul(p + 1, 10, &value);
			gCaptureOvlDownY = (unsigned int)value;
		...
		if (0 == strncmp(opt + 8, "on", 2)) {
			char *p = (char *)opt + 11;
			kstrtoul(p, 10, &value);
			gCaptureFBDownX = (unsigned int)value;
			kstrtoul(p + 1, 10, &value);
			gCaptureFBDownY = (unsigned int)value;
			kstrtoul(p + 1, 10, &value);
			gCaptureFBPeriod = (unsigned int)value;
			gCaptureFBEnable = 1;
