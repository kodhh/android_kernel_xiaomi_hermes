...
static struct list_head *zstd_alloc_workspace(unsigned int level)
{
	...
	size = max_t(size_t,
		...
		zstd_dstream_workspace_bound(ZSTD_BTRFS_MAX_WINDOWLOG));
...
static int zstd_decompress_biovec(struct list_head *ws,
				  ...
				  size_t srclen)
{
	...
	stream = zstd_init_dstream(
			ZSTD_BTRFS_MAX_WINDOWLOG, workspace->mem,
			workspace->size);
	if (!stream) {
	...
	while (1) {
		...
		ret = btrfs_decompress_buf2page(workspace->out_buf.dst,
				buf_start, total_out, disk_start,
				bvec, vcnt, &page_out_index, &pg_offset);
		if (ret == 0)
			break;
