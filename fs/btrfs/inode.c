...
again:
	...
	btrfs_set_file_extent_compression(leaf, item,
					  new->compress_type & 0xF);
...
static void btrfs_endio_direct_read(struct bio *bio, int error)
{
	...
	bio_set_err(dio_bio, bio_err(bio));
	dio_end_io(dio_bio, bio_err(bio));
...
static ssize_t check_direct_IO(struct btrfs_root *root, int rw,
			struct kiocb *iocb, const struct iov_iter *iter,
			loff_t offset)
{
	int seg;
	int i;
	...
	/* compute alignment manually (iov_iter_alignment not reliable in 3.10) */
	for (seg = 0; seg < iter->nr_segs; seg++) {
		if (iter->iov[seg].iov_len &&
		    ((unsigned long)iter->iov[seg].iov_base & blocksize_mask))
			goto out;
	}
				 ...
				 struct dentry *new_dentry)
{
	...
	old_dir->i_ctime = old_dir->i_mtime =
	new_dir->i_ctime = new_dir->i_mtime =
	old_inode->i_ctime = new_inode->i_ctime = current_time(old_inode);
