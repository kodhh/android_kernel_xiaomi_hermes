...
static void *ovl_follow_link(struct dentry *dentry, struct nameidata *nd)
{
	...
	old_cred = ovl_override_creds(dentry->d_sb);
	res = realdentry->d_inode->i_op->follow_link(realdentry, nd);
	ovl_revert_creds(old_cred);
	return res;
}

static void ovl_put_link(struct dentry *dentry, struct nameidata *nd, void *cookie)
{
	struct dentry *realdentry = ovl_dentry_real(dentry);
	const struct cred *old_cred;

	old_cred = ovl_override_creds(dentry->d_sb);
	if (realdentry->d_inode->i_op->put_link)
		realdentry->d_inode->i_op->put_link(realdentry, nd, cookie);
	ovl_revert_creds(old_cred);
}
...
ssize_t ovl_xattr_get(struct dentry *dentry, const char *name,
...
{
	return ovl_xattr_set(dentry, name, NULL, 0, XATTR_REPLACE);
}
...
struct inode *ovl_d_select_inode(struct dentry *dentry, unsigned int file_flags)
{
	int err;
	struct path realpath;
	enum ovl_path_type type;
...
static const struct inode_operations ovl_file_inode_operations = {
	...
	.getxattr	= ovl_xattr_get,
	.setxattr	= ovl_xattr_set,
...
static void ovl_fill_inode(struct inode *inode, umode_t mode)
{
	...
	inode->i_acl = inode->i_default_acl = NULL;
#endif
