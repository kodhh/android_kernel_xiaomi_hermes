...
smb_send_kvec(struct TCP_Server_Info *server, struct msghdr *smb_msg,
	      size_t *sent, struct kvec *iov, int n_vec, size_t total_len)
{
	int rc = 0;
	int retries = 0;
	...
	*sent = 0;
	...
	while (remaining > 0) {
		 ...
		 */
		rc = kernel_sendmsg(ssocket, smb_msg, iov, n_vec, remaining);
		if (rc == -EAGAIN) {
			retries++;
			if (retries >= 14 ||
...
__smb_send_rqst(struct TCP_Server_Info *server, struct smb_rqst *rqst)
{
	...
	rc = smb_send_kvec(server, &smb_msg, &sent, iov, n_vec, size);
	if (rc < 0)
		goto uncork;
	...
	for (i = 0; i < rqst->rq_npages; i++) {
		size_t len = i == rqst->rq_npages - 1
				? rqst->rq_tailsz
				: rqst->rq_pagesz;
		rc = kernel_sendpage(ssocket, rqst->rq_pages[i], 0, len,
				     smb_msg.msg_flags);
		if (rc < 0)
			break;
