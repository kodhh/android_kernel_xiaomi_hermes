...
static int
cifs_ci_hash_3_10(const struct dentry *dentry, const struct inode *inode,
		  struct qstr *q)
{
	return cifs_ci_hash(dentry, q);
}

static int
cifs_ci_compare_3_10(const struct dentry *dentry, const struct inode *inode,
		     const struct dentry *dentry2, const struct inode *inode2,
		     unsigned int len, const char *str, const struct qstr *name)
{
	return cifs_ci_compare(dentry, len, str, name);
}

const struct dentry_operations cifs_ci_dentry_ops = {
	.d_revalidate = cifs_d_revalidate,
	.d_hash = cifs_ci_hash_3_10,
	.d_compare = cifs_ci_compare_3_10,
	.d_automount = cifs_dfs_d_automount,
};
