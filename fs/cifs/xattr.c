...
static int
cifs_xattr_get_3_10(struct dentry *dentry, const char *name, void *buffer,
		    size_t size, int type)
{
	struct inode *inode = dentry->d_inode;
	return cifs_xattr_get(NULL, dentry, inode, name, buffer, size);
}

static int
cifs_xattr_set_3_10(struct dentry *dentry, const char *name, const void *value,
		    size_t size, int flags, int type)
{
	struct inode *inode = dentry->d_inode;
	return cifs_xattr_set(NULL, dentry, inode, name, value, size, flags);
}

static const struct xattr_handler cifs_user_xattr_handler = {
	.prefix = XATTR_USER_PREFIX,
	.flags = XATTR_USER,
	.get = cifs_xattr_get_3_10,
	.set = cifs_xattr_set_3_10,
};
