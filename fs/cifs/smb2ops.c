...
crypt_message(struct TCP_Server_Info *server, struct smb_rqst *rqst, int enc)
{
	...
	aead_request_set_assoc(req, &sg[0], assoc_data_len);
	aead_request_set_crypt(req, &sg[1], &sg[1], crypt_len, iv);
		 ...
		 unsigned int npages, unsigned int page_data_size)
{
	...
	if (buf_len <= data_offset) {
		...
		iov_iter_init(&iter, (const struct iovec *)bvec, npages, data_len, 0);
	} else if (buf_len >= data_offset + data_len) {
		/* read response payload is in buf */
		WARN_ONCE(npages > 0, "read data can be either in buf or in pages");
		{
			struct iovec iov_user;
			iov_user.iov_base = buf + data_offset;
			iov_user.iov_len = data_len;
			iov_iter_init(&iter, &iov_user, 1, data_len, 0);
		}
	} else {
