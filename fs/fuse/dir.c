...
/*
 * Create a submount for an automount point.
 */
struct vfsmount *fuse_dentry_automount(struct path *path)
{
	struct fuse_conn *fc = get_fuse_conn_super(path->mnt->mnt_sb);
	struct fuse_inode *mp_fi = get_fuse_inode(d_inode(path->dentry));
	struct fuse_submount_data d;
	struct vfsmount *mnt;

	d.fc = fc;
	d.parent_fi = mp_fi;

	mnt = vfs_kern_mount(&fuse_submount_fs_type, MS_KERNMOUNT,
			     "fuse", &d);
	if (IS_ERR(mnt))
		return mnt;
...
#ifdef CONFIG_FUSE_BPF
	{
		struct fuse_err_ret fer = fuse_bpf_backing(dir, struct fuse_lookup_io,
			fuse_lookup_initialize, fuse_lookup_backing, fuse_lookup_finalize,
			dir, entry, flags);
		if (fer.ret)
			return fer.result;
	}
...
#ifdef CONFIG_FUSE_BPF
	{
		struct fuse_err_ret fer = fuse_bpf_backing(dir, struct fuse_create_open_io,
			fuse_create_open_initialize, fuse_create_open_backing, fuse_create_open_finalize,
...
#ifdef CONFIG_FUSE_BPF
	{
		struct fuse_err_ret fer = fuse_bpf_backing(dir, struct fuse_mknod_in,
			fuse_mknod_initialize, fuse_mknod_backing, fuse_mknod_finalize,
			dir, entry, mode, rdev);
		if (fer.ret)
			return fer.result ? PTR_ERR(fer.result) : 0;
	}
...
int fuse_tmpfile(struct inode *dir, struct dentry *entry, umode_t mode)
{
	...
	if (fc->minor < 9)
		...
		req->out.args[0].size = sizeof(outarg);
	req->out.args[0].value = &outarg;
	req->out.args[1].size = sizeof(open_out);
	req->out.args[1].value = &open_out;
...
#ifdef CONFIG_FUSE_BPF
	{
		struct fuse_err_ret fer = fuse_bpf_backing(dir, struct fuse_mkdir_in,
			fuse_mkdir_initialize, fuse_mkdir_backing, fuse_mkdir_finalize,
			dir, entry, mode);
		if (fer.ret)
			return fer.result ? PTR_ERR(fer.result) : 0;
	}
...
#ifdef CONFIG_FUSE_BPF
	{
		unsigned link_len = len;
		struct fuse_err_ret fer = fuse_bpf_backing(dir, struct fuse_dummy_io,
			fuse_symlink_initialize, fuse_symlink_backing, fuse_symlink_finalize,
			dir, entry, link, link_len);
		if (fer.ret)
			return fer.result ? PTR_ERR(fer.result) : 0;
	}
#endif
...
static int fuse_unlink(struct inode *dir, struct dentry *entry)
{
	...
	{
		struct fuse_err_ret fer = fuse_bpf_backing(dir, struct fuse_dummy_io,
			fuse_unlink_initialize, fuse_unlink_backing, fuse_unlink_finalize,
			dir, entry);
		if (fer.ret)
			return fer.result ? PTR_ERR(fer.result) : 0;
	}
...
static int fuse_rmdir(struct inode *dir, struct dentry *entry)
{
	...
	{
		struct fuse_err_ret fer = fuse_bpf_backing(dir, struct fuse_dummy_io,
			fuse_rmdir_initialize, fuse_rmdir_backing, fuse_rmdir_finalize,
			dir, entry);
		if (fer.ret)
			return fer.result ? PTR_ERR(fer.result) : 0;
	}
...
#ifdef CONFIG_FUSE_BPF
	{
		struct fuse_err_ret fer = fuse_bpf_backing(newdir, struct fuse_link_in,
			fuse_link_initialize, fuse_link_backing, fuse_link_finalize,
			entry, newdir, newent);
		if (fer.ret)
			return fer.result ? PTR_ERR(fer.result) : 0;
	}
...
#ifdef CONFIG_FUSE_BPF
	{
		struct fuse_err_ret fer = fuse_bpf_backing(inode, struct fuse_access_in,
			fuse_access_initialize, fuse_access_backing, fuse_access_finalize,
			inode, mask);
		if (fer.ret)
			return fer.result ? PTR_ERR(fer.result) : 0;
	}
...
#ifdef CONFIG_FUSE_BPF
	{
		struct fuse_err_ret fer = fuse_bpf_backing(entry->d_inode, struct fuse_getattr_io,
			fuse_getattr_initialize, fuse_getattr_backing, fuse_getattr_finalize,
			entry, stat, 0, 0);
		if (fer.ret) {
			if (IS_ERR(fer.result))
				return PTR_ERR(fer.result);
	       ...
	       u32 request_mask, unsigned int flags)
{
	...
	memset(&inarg, 0, sizeof(inarg));
	memset(&outarg, 0, sizeof(outarg));
	inarg.sx_flags = flags & 0x03;
	inarg.sx_mask = request_mask;
	inarg.fh = get_node_id(inode);

	req->in.numargs = 1;
	req->in.args[0].size = sizeof(inarg);
	req->in.args[0].value = &inarg;
	req->in.h.opcode = FUSE_STATX;
	req->in.h.nodeid = get_node_id(inode);
	req->out.numargs = 1;
	req->out.args[0].size = sizeof(outarg);
	req->out.args[0].value = &outarg;
	...
	stat->mode = outarg.stat.mode;
	stat->nlink = outarg.stat.nlink;
	stat->uid = make_kuid(&init_user_ns, outarg.stat.uid);
	stat->gid = make_kgid(&init_user_ns, outarg.stat.gid);
	stat->ino = outarg.stat.ino;
	stat->size = outarg.stat.size;
	stat->blocks = outarg.stat.blocks;
	stat->blksize = outarg.stat.blksize;
	stat->rdev = MKDEV(outarg.stat.rdev_major, outarg.stat.rdev_minor);
	stat->dev = MKDEV(outarg.stat.dev_major, outarg.stat.dev_minor);
	stat->atime.tv_sec = outarg.stat.atime.tv_sec;
	stat->atime.tv_nsec = outarg.stat.atime.tv_nsec;
	stat->mtime.tv_sec = outarg.stat.mtime.tv_sec;
	stat->mtime.tv_nsec = outarg.stat.mtime.tv_nsec;
	stat->ctime.tv_sec = outarg.stat.ctime.tv_sec;
	stat->ctime.tv_nsec = outarg.stat.ctime.tv_nsec;
