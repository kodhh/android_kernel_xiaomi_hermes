...
static inline bool is_allow_su()
{
	...
	return ksu_is_allow_uid(ksu_current_uid_val());
}
...
void escape_to_root(void)
{
	...
	struct root_profile *profile = ksu_get_root_profile(ksu_cred_uid(cred));
		     ...
		     unsigned long arg4, unsigned long arg5)
{
	...
	uid_t current_uid_val = ksu_current_uid_val();
	...
	bool from_root = 0 == ksu_current_uid_val();
...
static bool is_non_appuid(ksu_kuid_t uid)
{
#define PER_USER_RANGE 100000
#define FIRST_APPLICATION_UID 10000

	uid_t appid = ksu_from_kuid(uid) % PER_USER_RANGE;
...
int ksu_handle_setuid(struct cred *new, const struct cred *old)
{
	...
	ksu_kuid_t new_uid = ksu_cred_uid(new);
	ksu_kuid_t old_uid = ksu_cred_uid(old);
